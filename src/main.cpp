...
#define DEFAULT_MAX_NUMBER_OF_PACKETS_TO_FLUSH 32
...
private:
	...
	// shared pointer to log file stream
	SPofStream spLogFileStream;
...
public:
	...
	// max number of captured packets
	...
	/**
	 * Get a log file stream. Open a new one if there is none.
	 * Return value is a shared pointer to the new file stream
	 */
	SPofStream getLogFileStream()
	{
		if (nullptr == spLogFileStream)
		{
			std::stringstream stream;
			...
			spLogFileStream = std::make_shared<std::ofstream>(stream.str().c_str(), std::ios_base::binary);
		}

		return spLogFileStream;
	}
	 ...
	 */
	void closeLogFileSteam()
	{
		if (nullptr != spLogFileStream)
		{
		// close the file stream
			spLogFileStream->close();
			spLogFileStream.reset();
			spLogFileStream = nullptr;
		}
	}
...
struct TcpSorterData
{
	...
	uint64_t numOfDataPackets[2];
	uint64_t numOfMessagesFromSide[2];
...
static void tcpPacketMissingCallback(int sideIndex, const ConnectionData& connData, uint32_t seq, uint32_t length, void* userCookie)
{
	...
	std::string dir = sideIndex == 0? " => " : " <= ";
	SPofStream fileStream = GlobalConfig::getInstance().getLogFileStream();
	*fileStream<< "Found missing packet: " << "side(" << sideIndex <<"), "
				<< sourceIP << ":" << connData.srcPort
				<< dir
				<< destIP <<":" << connData.dstPort
				<< ", seq(" << seq <<"), len(" << length <<")"
